...
from analyzer import RMTanalyzer, group
...
class RMT:
    ...
    def generate_cnn_rep(self): # this is for random matrix theory paper
        datagenerator = DataGenerator(ganmodel = BigGAN, classes=self.args.classes, num_imgs_per_class=self.args.num_imgs_per_class, \
            z_sampler_args=self.args.z_sampler_args, device=self.args.device,\
                truncation = self.args.truncation)
        
        imgname = self.args.z_sampler_args['name']

        file_name='images/{}/{}'.format(imgname, self.args.classes[0])
        current_file_name = file_name + '_%d.png' % (self.args.num_imgs_per_class-1)
        
        _, imgs = datagenerator.generate_images()
        data, _ = datagenerator.represent_images(rep_model_name=self.args.rep_model, image_type=self.args.image_type, pretrained=True)
    ...
    def eig_distribution(self): ## here we loop every exp_n, this analysis is to look at the distribution of cnn rep
        ...
        evs = []
        range_num = 1
        eig_file_name = 'data/{}/{}/eig_distribution/e.npy'.format(self.args.gan_name,self.args.run)
        ...
        if opsys != 'linux':
            ...
            file_name_head = 'matrices_{}_'.format(esd.shape[0])
    ...
    def draw_evalu(self,file_name_head,existed_x):        
...
if __name__ == '__main__': 
    ...
    args.num_gram_imgs_per_class = 17 #default is 170
    args.truncation = 0.1
    ...
    for rep_model in ['resnet50', 'vgg16', 'densenet201']:
