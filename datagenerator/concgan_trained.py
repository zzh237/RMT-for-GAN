...
class DataGenerator(object):
    def __init__(self, args):
        ...
        self.gan = BigGAN.from_pretrained('biggan-deep-256') #here we initialize the GAN model we are interested
    ...
    def represent_images(self, save_representations=True):
        ...
        for i, label in enumerate(self.classes):
            ...
            if image_type == 'GAN':
                image_names = ['images/%s/%s_%d.png' % (self.sample_method, label, i) for i in range(self.num_imgs_per_class)]
            ...
            if save_representations:
                np.save('data/%s_%s_%s_%s_%s.npy' % (self.sample_method, image_type, label, rep_model_name, str(self.truncation)), np.asarray(reps))
